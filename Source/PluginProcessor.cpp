...
{
    int menu2Choice = *tree.getRawParameterValue("filter2Type");
    int freq2 = *tree.getRawParameterValue("filter2Cutoff");
    int res2 = *tree.getRawParameterValue("filter2Res");
    
    if (menu2Choice == 0)
    {
        stateVariableFilter.state->type = dsp::StateVariableFilter::Parameters<float>::Type::lowPass;
        stateVariableFilter.state->setCutOffFrequency(lastSampleRate, freq2, res2);
    }
...
void JuceSynthFrameworkAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < mySynth.getNumVoices(); i++)
    {
        ...
        if ((myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i))))
        {
            ...
            myVoice->getFilter2Params(tree.getRawParameterValue("filter2Type"),
                                     tree.getRawParameterValue("filter2Cutoff"),
                                     tree.getRawParameterValue("filter2Res"));
